...
//  Copy Folder / File
//=======================
void copy_file_with_overwrite(const fs::path& from, const fs::path& to, const fs::path& path_ff,
...
void copy_folder_with_overwrite(const fs::path& from, const fs::path& to, const fs::path& path_ff,
                              const std::string& parameter) {
     fs::path source = HFILEF::get_fetch_full_path(from, path_ff);
     fs::path target = HFILEF::get_fetch_full_path(to, path_ff);
...
void copy::copy_file(const fs::path& from, const fs::path& to,
     ...
     try {
          copy_file_with_overwrite(from, to, path_ff, parameter);
...
void copy::copy_folder(const fs::path& from, const fs::path& to,
     ...
     try {
          copy_folder_with_overwrite(from, to, path_ff, parameter);
     }
     catch (const fs::filesystem_error& e) {
          if (e.code() == std::errc::file_exists) {
